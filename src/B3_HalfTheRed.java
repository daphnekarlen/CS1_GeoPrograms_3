public class B3_HalfTheRed extends World {
    ...
    public void go() {
        smallSquare();
        if(red==171 && green==57 && blue==51) //&&green<240 && blue< 100)
        {
...
public void smallSquare(){
    ...
    System.out.println(plane.howMuchRed()); // howMuchRed is a method that tells us the "r" value of the planes current pixel

    for (int row = 140; row < 220; row = row + 1) {
            for (int col = 120; col < 200; col++) { // col++ is the same as col=col+1 and col+=1
